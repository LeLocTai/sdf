...
from .settings import BOUNDARY, BIAS, BIAS_STRENGTH, CLIP
...
NB: Doesn't work with fixed axes."""
    ...
    def make_3d(positions, normals):
        ...
        b = [
            ...
            for v, n in zip(positions, normals)
        ]
...
and returns a tuple of the error squared and the point itself"""
        result, residual, rank, s = numpy.linalg.lstsq(
...
def solve_qef_3d(x, y, z, positions, normals):
    ...
    if BOUNDARY:
        ...
        def inside(r):
            return x <= r[1][0] <= x + 1 and y <= r[1][1] <= y + 1 and z <= r[1][
                2] <= z + 1
